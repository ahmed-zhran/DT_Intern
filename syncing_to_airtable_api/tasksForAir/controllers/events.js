...
exports.event_create = PromiseHandler(async (req, res, next) => {
  ...
  var file;
  let prefix = "img-" + Date.now();
  let filename = "";
  if (req.files) {
    file = req.files.image;
    filename = `uploads/${prefix}-${req.files.image.name}`;
...
exports.event_update = PromiseHandler(async (req, res, next) => {
  ...
  req.body.lastUpdatedAt = new Date();
  let e = await events.findOne({ _id: id });
  if (file) {
    fs.unlink(e.image, (err) => console.log(err));
  }
...
exports.allEvents_details = PromiseHandler(async (req, res, next) => {
  ...
  if (!wanted.length)
    res.status(404).json({
      msg: "no data in this page",
      allcount: cnt,
      availablePages: Math.ceil(cnt / req.body.limit),
    });
  else
    res.status(200).send({
      allcount: cnt,
      pageNo: req.body.page,
      limitperpage: req.body.limit,
      data: wanted,
    });
});
...
async function update() {
  ...
  let updatingdata = await events
    .find({
      ...
      updated: { $exists: true, $in: true },
    })
    .toArray();
  if (updatingdata.length) {
    updates = updatingdata.length;
    await Promise.all(
      updatingdata.map(async (event) => {
        try {
          ...
          await syncingerrors.setError(
            err,
            `updating function - in mapping data - updating ${event.recordID} in airtable to ${event._id} in db`
          );
          return;
        }
      })
    ...
    await Promise.all(
      addingdata.map(async (event) => {
        try {
          ...
          await events.findOneAndUpdate(
            { _id: id },
            { $set: { recordID: await addedEv.getId(), updated: false } },
            { new: true, upsert: false }
          );
        } catch (err) {
          await syncingerrors.setError(
            err,
            `creating function - in mapping data - adding ${event._id} from db  to airtable ::${event.recordID}`
          );
          return;
        }
      })
...
async function remove() {
  let removed = 0;
  let removedata = await events.find({ deleted: { $exists: true } }).toArray();
  if (removedata.length) {
    removed = removedata.length;
    await Promise.all(
      removedata.map(async (event) => {
        try {
          if (event.recordID)
          ...
          await syncingerrors.setError(
            err,
            `removing function - in mapping data - removing ${event.recordID} and ${event._id} from airtable and db respectively`
          );
          return;
        }
      })
...
exports.sync_to_airtable = PromiseHandler(async (req, res, next) => {
  ...
  Promise.all([await remove(), await update(), await add()]).then(
    async (value) => {
      let response = {};
      //count of deleted
      ...
      if (syncingerrors.errors.length)
        res
          .status(400)
          .json({ catchedErrors: syncingerrors, response: response });
...
async function update2() {
  ...
  let updatingdata = await fetch(
    `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE}/sync%20with%20events%20db?filterByFormula=AND(%7Bdeleted%7D%3D0%2C%7Bupdated%7D%3D1%2CNOT(%7B_id%7D%3D%22null%22))`,
    {
      headers: {
  ...
  if (updatingdata.records.length) {
    ...
    await Promise.all(
      updatingdata.records.map(async (event) => {
        try {
          ...
          await syncingerrors.setError(
            err,
            `updating function - in mapping data - updating ${event.fields._id} in db to ${event.id} in airtable`
          );
          return;
        }
      })
...
async function add2() {
  ...
  let addingdata = await fetch(
    `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE}/sync%20with%20events%20db?filterByFormula=AND(%7Bdeleted%7D%3D0%2C%7B_id%7D%3D%22null%22)`,
    {
      headers: {
        Authorization: "Bearer " + process.env.AIRTABLE_API_KEY,
        "content-type": "application/json",
  ...
  if (addingdata.records.length) {
    ...
    Promise.all(
      addingdata.records.map(async (event) => {
        try {
          let obj = {
          ...
          await syncingerrors.setError(
            err,
            `creating function - in mapping data - adding ${event.id} from airtable  to db ::${event.fields._id}`
          );
          return;
        }
      })
...
async function remove2() {
  let removed = 0;
  //query deleted = true
  let removedata = await fetch(
    `https://api.airtable.com/v0/${process.env.AIRTABLE_BASE}/sync%20with%20events%20db?filterByFormula=%7Bdeleted%7D%3D1`,
    {
      headers: {
        Authorization: "Bearer " + process.env.AIRTABLE_API_KEY,
        "content-type": "application/json",
  ...
  if (removedata.records.length) {
    ...
    await Promise.all(
      removedata.records.map(async (event) => {
        try {
          if (event.fields._id) {
          ...
          await syncingerrors.setError(
            err,
            `removing function - in mapping data - removing ${event.id} and ${event.fields._id} from airtable and db respectively`
          );
          return;
        }
      })
...
exports.sync_to_db = PromiseHandler(async (req, res, next) => {
  ...
  Promise.all([await remove2(), await update2(), await add2()]).then(
    async (value) => {
      let response = {};
      //count of deleted
      response.deletedData = value[0];
